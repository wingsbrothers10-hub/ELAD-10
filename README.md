[dashboard_final.html](https://github.com/user-attachments/files/25605739/dashboard_final.html)
<!DOCTYPE html>
...
<header>
  ...
  <div class="hright">
    <div class="pill pill-live"><div class="pulse"></div> מחובר</div>
...
async function fetchLeads(){
  ...
  try{const r=await fetch(N8N_URL);if(r.ok)applyLeads(await r.json())}
  catch(e){console.warn('Fetch failed:',e);applyLeads([])}
}
function applyLeads(d){leads=(Array.isArray(d)?d:[]).map(l=>{if(typeof l.answers==='string'){try{l.answers=JSON.parse(l.answers)}catch(e){l.answers={}}}return l});updateKPI();updateCharts();updateAlert();updateCounts();render()}
